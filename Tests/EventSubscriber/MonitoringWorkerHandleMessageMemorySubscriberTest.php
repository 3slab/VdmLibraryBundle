...
use Vdm\Bundle\LibraryBundle\EventSubscriber\MonitoringWorkerHandleMessageMemorySubscriber;
...
class MonitoringWorkerHandleMessageMemorySubscriberTest extends TestCase
{
    public function testOnWorkerMessageReceived()
    {
        ...
        $listener = new MonitoringWorkerHandleMessageMemorySubscriber($statsStorageInterface, new NullLogger());
        $listener->onWorkerMessageReceived($event);
                
        $property = new \ReflectionProperty(MonitoringWorkerHandleMessageMemorySubscriber::class, 'stopwatch');
    ...
    public function testOnWorkerMessageFailed()
    {
        ...
        $statsStorageInterface = $this->getMockBuilder(StatsStorageInterface::class)->getMock();
        $statsStorageInterface->expects($this->once())->method('sendMemoryStat');
        $listener = new MonitoringWorkerHandleMessageMemorySubscriber($statsStorageInterface, new NullLogger());
